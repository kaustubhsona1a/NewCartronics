      ...
      <div className="container mx-auto max-w-7xl px-4">
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          <div className={`w-full lg:w-72 flex-shrink-0 ${isMobileFiltersOpen ? 'block' : 'hidden lg:block'}`}>
            <div className="p-8 border border-white/25 bg-white/5 backdrop-blur-xl rounded-2xl shadow-lg shadow-black/10 sticky top-28">
              <div className="flex items-center justify-between mb-8 pb-4 border-b border-white/25">
                <h3 className="font-serif tracking-widest text-white flex items-center uppercase text-xs font-bold font-mono"><Filter className="w-4 h-4 mr-3 text-white" /> Filters & Sort</h3>
                <button onClick={resetFilters} className="text-[9px] tracking-widest uppercase text-zinc-400 hover:text-white transition-colors font-bold font-mono">Reset</button>
              </div>
              ...
              <div className="space-y-8 text-zinc-300">
                ...
                <div>
                  ...
                  <div className="relative">
                    <select 
                      className="w-full bg-zinc-950/40 border border-white/25 text-xs tracking-wider text-zinc-300 uppercase rounded-xl px-4 py-3.5 outline-none focus:border-white focus:bg-zinc-950/80 transition-colors block shadow-sm font-mono cursor-pointer"
                      value={sortBy}
                ...
                {/* Owners */}
                <div>
                  <h4 className="text-[10px] uppercase tracking-wider text-zinc-400 mb-4 font-bold font-mono border-b border-white/25 pb-1.5 flex items-center justify-between">
                    <span>Ownership</span>
                    <span className="w-1.5 h-1.5 rounded-full bg-white/40 animate-pulse"></span>
                  </h4>
                    ...
                    {['1st Owner', '2nd Owner', '3rd Owner'].map(owner => {
                      ...
                      return (
                        <label key={owner} className="flex items-center space-x-3 cursor-pointer group">
                          <div className={`relative flex items-center justify-center w-4 h-4 rounded border transition-colors ${isSelected ? 'border-white bg-white' : 'border-white/25 group-hover:border-white'}`}>
                              <input 
                                type="checkbox" 
              ...
              {loading ? (
                ...
                filteredCars.map((car, idx) => {
                  const cardHoverStyles = [
                    "hover:border-white/60 hover:shadow-2xl hover:shadow-white/15 hover:-translate-y-3 hover:scale-[1.02] hover:bg-white/10",
                    "hover:border-zinc-300/60 hover:shadow-2xl hover:shadow-zinc-300/15 hover:-translate-y-3 hover:scale-[1.02] hover:bg-white/10",
                    "hover:border-zinc-400/60 hover:shadow-2xl hover:shadow-zinc-400/15 hover:-translate-y-3 hover:scale-[1.02] hover:bg-white/10"
                  ];
                      ...
                      <div className={`bg-white/5 border border-white/25 backdrop-blur-xl ${hoverStyle} transition-all duration-300 ease-out flex flex-col h-full overflow-hidden rounded-2xl shadow-lg`}>
                        ...
                        <div className="p-8 flex-grow flex flex-col justify-between text-zinc-300">
                          <div>
                            ...
                            <div className="text-2xl font-bold text-center text-white mb-6 pb-6 border-b border-white/25 font-serif">{formatPrice(car.price)}</div>
                          </div>
                          ...
                          <div>
                            <div className="flex flex-wrap justify-center gap-x-5 gap-y-2 text-xs md:text-sm font-semibold text-zinc-300 mb-6 font-sans">
                              <div className="flex items-center"><Gauge className="w-4 h-4 mr-1.5 text-zinc-400" /> {car.mileage.toLocaleString()} KM</div>
                              <div className="flex items-center"><Fuel className="w-4 h-4 mr-1.5 text-zinc-400" /> {car.fuelType}</div>
                              <div className="flex items-center"><Cog className="w-4 h-4 mr-1.5 text-zinc-400" /> {car.transmission}</div>
                            </div>
                            
                            <div className="w-full uppercase tracking-widest text-zinc-950 text-[9px] font-bold text-center py-3 border border-transparent bg-white group-hover:bg-zinc-100 group-hover:scale-[1.02] group-hover:shadow-md transition-all duration-300 rounded-xl font-mono">
