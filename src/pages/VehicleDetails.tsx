...
export default function VehicleDetails() {
  ...
  if (!car) {
    ...
    return (
      <div className="min-h-screen bg-transparent flex items-center justify-center p-20 text-zinc-650">
        <Helmet>
          <title>Vehicle Not Found | Cartronics</title>
  ...
  const handleWhatsApp = () => {
    const message = encodeURIComponent(`Hi, I'm interested in the ${car.year} ${car.make} ${car.model} (${car.variant}) listed at ${formatPrice(car.price)} on Cartronics website. Please share more details.`);
    window.open(`https://wa.me/919076012999?text=${message}`, '_blank');
  };

  const handleWhatsAppPhotos = () => {
    const message = encodeURIComponent(`Hi, I'm interested in the ${car.year} ${car.make} ${car.model} (${car.variant}) listed at ${formatPrice(car.price)} on Cartronics website. Please share more photos of this vehicle.`);
    window.open(`https://wa.me/919076012999?text=${message}`, '_blank');
  };

  const handleCall = () => {
    window.open(`tel:+919076012999`);
  };
  ...
  const renderPriceBox = (isMobile: boolean) => {
    ...
    return (
      <div className={`bg-zinc-900/55 border border-zinc-900/80 relative rounded-2xl shadow-sm backdrop-blur-md ${isMobile ? 'p-5' : 'p-10'}`}>
        ...
        <p className={`tracking-wider text-zinc-400 flex items-center font-mono font-semibold ${isMobile ? 'text-[10px] mb-4' : 'text-xs mb-8'}`}>
          <a href="https://www.google.com/maps/place/Cartronics+-+Car+Dealers/@19.0232457,73.0226216,17z/data=!3m1!4b1!4m6!3m5!1s0x3be7c3df9dbfffff:0x66315419caf903f0!8m2!3d19.0232457!4d73.0226216!16s%2Fg%2F11h_tv31lj?entry=ttu&g_ep=EgoyMDI2MDYyNC4wIKXMDSoASAFQAw%3D%3D" target="_blank" rel="noreferrer" className="hover:text-white transition-colors duration-300 inline-flex items-center">
            <MapPin className="w-4 h-4 mr-2 text-zinc-400" /> Showroom, Mumbai
          </a>
        </p>
  ...
  const pageTitle = `${car.year} ${car.make} ${car.model} ${car.variant} | Cartronics`;
      ...
      <div className="container mx-auto max-w-7xl px-4">
        ...
        <div className="mt-6 md:mt-10 bg-zinc-900/55 p-5 md:p-10 border border-zinc-900/80 rounded-2xl relative overflow-hidden group hover:border-white/30 transition-colors duration-500 shadow-sm backdrop-blur-md">
          ...
          <p className="text-zinc-400 mb-2 max-w-xl text-xs md:text-sm font-light leading-relaxed">
            Every vehicle listed by the Cartronics platform undertakes a specialized physical inspection covering diagnostic checks, accident/flood history, mileage validation, and comprehensive ownership verification.
          </p>
        </div>
          ...
          <div className="bg-zinc-900/90 border border-zinc-800/80 rounded-2xl p-6 sm:p-8 max-w-md w-full relative z-10 shadow-2xl animate-in fade-in zoom-in-95 duration-200 backdrop-blur-xl text-zinc-300">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-3">
                <button
                  onClick={() => {
                    const msg = `Check out this gorgeous ${car.year} ${car.make} ${car.model} listed at ${formatPrice(car.price)} on Cartronics!`;
                ...
                <button
                  onClick={() => {
                    ...
                    const body = `Hey, take a look at this exceptional pre-owned ${car.year} ${car.make} ${car.model} listed at ${formatPrice(car.price)} on the Cartronics website: ${window.location.href}`;
